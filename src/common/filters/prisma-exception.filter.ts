    ...
    private readonly logger = new Logger(PrismaClientExceptionFilter.name);
        ...
        switch (exception.code) {
            case 'P2002': {
                ...
                errorResponse = new ConflictException(`Unique constraint failed on field(s): ${target || 'unknown'}`);
                break;
            }
            case 'P2025': {
                // Record to update or delete not found
                // The message might contain details, but often Prisma's default is enough
                errorResponse = new NotFoundException(exception.meta?.cause || 'Required record not found.');
                break;
            }
