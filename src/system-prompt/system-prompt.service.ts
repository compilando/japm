...
export class SystemPromptService {
    ...
    async findOneByName(name: string): Promise<SystemPrompt> {
        ...
        if (!prompt) {
            throw new NotFoundException(`System prompt with name "${name}" not found.`);
        }
        // Resolve file directive before returning
        return this._resolvePromptObject(prompt);
    }

    async update(name: string, dto: UpdateSystemPromptDto): Promise<SystemPrompt> {
    ...
    private async _resolvePromptObject(prompt: SystemPrompt): Promise<SystemPrompt> {
    ...
    private async _resolvePromptText(text: string): Promise<string> {
        ...
        if (match && match[1]) {
            ...
            this.logger.log(`Resolving system prompt from file: ${relativePath} -> ${absolutePath}`);

            try {
                // Security check: Ensure path doesn't try to escape the project root or intended dir (basic check)
                const projectRoot = process.cwd();
                if (!absolutePath.startsWith(projectRoot) || absolutePath.includes('..')) {
                    this.logger.error(`Security Risk: Attempted file access outside project root or using .. : ${relativePath}`);
                    throw new InternalServerErrorException(`Invalid file path specified for system prompt.`);
                }

                const fileContent = await fs.readFile(absolutePath, 'utf8');
                this.logger.debug(`Successfully loaded prompt text from ${relativePath}`);
                return fileContent;
            } catch (error) {
                if (error.code === 'ENOENT') {
                    this.logger.error(`File not found for system prompt directive: ${absolutePath} (from ${relativePath})`);
                    throw new NotFoundException(`System prompt file not found: ${relativePath}`);
                } else {
                    this.logger.error(`Failed to read system prompt file ${absolutePath}: ${error.message}`, error.stack);
                    throw new InternalServerErrorException(`Failed to load system prompt from file: ${relativePath}`);
                }
            }
        } else {
