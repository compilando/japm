...
export class SystemPromptController {
    ...
    @ApiOperation({ summary: 'Create a new system prompt (Admin Only - conceptually)' })
    @ApiBody({ type: CreateSystemPromptDto })
    @ApiResponse({ status: 201, description: 'System prompt created successfully.' })
    ...
    @ApiParam({ name: 'name', description: 'Unique name of the system prompt', type: String })
    ...
    @UseGuards(JwtAuthGuard) // TODO: Replace/add AdminGuard for production
    @UsePipes(new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
    @ApiOperation({ summary: 'Update an existing system prompt (Admin Only - conceptually)' })
    @ApiParam({ name: 'name', description: 'Unique name of the system prompt to update', type: String })
    @ApiBody({ type: UpdateSystemPromptDto })
    @ApiResponse({ status: 200, description: 'System prompt updated successfully.' })
    @ApiResponse({ status: 400, description: 'Invalid input data.' })
    ...
    @Delete(':name')
    @UseGuards(JwtAuthGuard) // TODO: Replace/add AdminGuard for production
    @ApiOperation({ summary: 'Delete a system prompt (Admin Only - conceptually)' })
    @ApiParam({ name: 'name', description: 'Unique name of the system prompt to delete', type: String })
    @ApiResponse({ status: 200, description: 'System prompt deleted successfully.' })
    @ApiResponse({ status: 404, description: 'System prompt not found.' })
    @ApiResponse({ status: 401, description: 'Unauthorized.' })
