...
export class PromptController {
    ...
    @ApiResponse({ status: 400, description: 'Bad Request (e.g., missing user prompt).' })
    @ApiResponse({ status: 401, description: 'Unauthorized.' })
    @ApiResponse({ status: 404, description: 'Project not found.' })
    @ApiResponse({ status: 500, description: 'Internal Server Error (LLM failure, file read error, etc.).' })
