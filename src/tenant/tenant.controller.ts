...
export class TenantController {
    ...
    @ApiResponse({ status: HttpStatus.CREATED, description: 'Tenant created successfully.', type: TenantDto })
    @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid input.' })
    @ApiResponse({ status: HttpStatus.CONFLICT, description: 'Tenant name already exists.' })
    ...
    @ApiResponse({ status: HttpStatus.OK, description: 'Array of tenants.', type: [TenantDto] })
    @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized.' })
    @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Forbidden resource.' })
    ...
    @ApiResponse({ status: HttpStatus.OK, description: 'Tenant object.', type: TenantDto })
    @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Tenant not found.' })
    @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized.' })
    @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Forbidden resource.' })
    @ApiParam({ name: 'tenantId', type: 'string', format: 'uuid', description: 'The ID of the tenant' })
    async findOne(
    ...
    @ApiResponse({ status: HttpStatus.OK, description: 'Tenant updated successfully.', type: TenantDto })
    @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Tenant not found.' })
    @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid input.' })
    @ApiResponse({ status: HttpStatus.CONFLICT, description: 'Tenant name already exists.' })
    @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized.' })
    ...
    @ApiOperation({ summary: 'Delete a tenant by ID (Caution: Destructive operation)' })
    @ApiResponse({ status: HttpStatus.NO_CONTENT, description: 'Tenant deleted successfully.' })
    @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Tenant not found.' })
    @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized.' })
    @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Forbidden resource.' })
    @ApiParam({ name: 'tenantId', type: 'string', format: 'uuid', description: 'The ID of the tenant' })
    async remove(@Param('tenantId', ParseUUIDPipe) tenantId: string): Promise<void> {
