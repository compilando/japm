...
export class TenantService {
    ...
    async create(createTenantDto: CreateTenantDto): Promise<Tenant> {
        const { name, /* description, */ marketplaceRequiresApproval, initialAdminUser } = createTenantDto;
        ...
        try {
            return await this.prisma.$transaction(async (tx) => {
                ...
                this.logger.log(`Initial admin user created for tenant ${newTenant.id}: ${adminUserDto.email}`);

                return newTenant;
            });
        } catch (error) {
            if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
                this.logger.error(`Unique constraint violation during tenant creation for name "${name}": ${error.message}`, error.stack, error.meta);
                throw new ConflictException(`Tenant creation failed. A tenant with name "${name}" might already exist or the initial admin email is already in use.`);
            }
            this.logger.error(`Error creating tenant "${name}": ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async update(id: string, updateTenantDto: UpdateTenantDto): Promise<Tenant> {
        const { name, /* description, */ marketplaceRequiresApproval } = updateTenantDto;
        this.logger.log(`Updating tenant with ID: ${id} with data: ${JSON.stringify(updateTenantDto)}`);
        ...
        if (name && name !== existingTenant.name) {
            ...
            if (tenantWithNewName && tenantWithNewName.id !== id) {
                throw new ConflictException(`Another tenant with name "${name}" already exists.`);
            }
        }
        ...
        try {
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
                this.logger.error(`Unique constraint violation (likely name) updating tenant ID '${id}' to name '${name}': ${error.message}`, error.stack, error.meta);
                throw new ConflictException(`Update failed. A tenant with name "${name}" might already exist.`);
            }
            this.logger.error(`Error updating tenant ID '${id}': ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async remove(id: string): Promise<Tenant> {
        ...
        try {
            ...
            this.logger.error(`Error removing tenant ID '${id}': ${error.message}`, error.stack);
            throw error;
        }
    }
