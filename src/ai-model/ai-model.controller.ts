...
export class AiModelController {
    ...
    @ApiResponse({ status: 201, description: 'The AI model has been successfully created.', type: AiModelResponseDto })
    ...
    create(
        ...
        this.logger.debug(`[create] Received request for projectId: ${projectId}. Body: ${JSON.stringify(createAiModelDto, null, 2)}`); // Log the received DTO
    ...
    @ApiResponse({ status: 200, description: 'List of project-specific and global AI models.', type: [AiModelResponseDto] })
    ...
    @ApiResponse({ status: 200, description: 'The found AI model record', type: AiModelResponseDto })
    ...
    @ApiParam({ name: 'projectId', description: 'Project ID' })
    @ApiParam({ name: 'aiModelId', description: 'AI Model CUID' })
    @ApiResponse({ status: 200, description: 'The AI model has been successfully updated.', type: AiModelResponseDto })
    ...
    update(
        ...
        this.logger.debug(`[update] Received PATCH for projectId: ${projectId}, aiModelId: ${aiModelId}. Body: ${JSON.stringify(updateAiModelDto, null, 2)}`); // Log the received DTO
    ...
    @ApiParam({ name: 'projectId', description: 'Project ID' })
    @ApiParam({ name: 'aiModelId', description: 'AI Model CUID' })
    @ApiResponse({ status: 200, description: 'The AI model has been successfully deleted.', type: AiModelResponseDto })
    @ApiResponse({ status: 404, description: 'AI Model not found.' })
    remove(
        @Param('projectId') projectId: string,
        @Param('aiModelId') aiModelId: string
