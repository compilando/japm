...
import { Prisma, ConversationPrompt, ConversationPromptVersion, PromptTranslation, Tag } from '@prisma/client';

// Tipo extendido para la respuesta completa de findOne
type PromptWithRelations = Prisma.ConversationPromptGetPayload<{
    include: {
        tactic: true;
        tags: true;
        activeVersion: {
...
type PromptWithInitialVersionAndTags = Prisma.ConversationPromptGetPayload<{
    include: {
        tactic: true;
        tags: true;
...
export class ConversationPromptService {
    ...
    async create(createDto: CreateConversationPromptDto): Promise<PromptWithInitialVersionAndTags> {
        ...
        let newPrompt: ConversationPrompt;
        try {
            newPrompt = await this.prisma.conversationPrompt.create({
                data: {
        ...
        let newVersion: ConversationPromptVersion;
        try {
            newVersion = await this.prisma.conversationPromptVersion.create({
                data: {
            ...
            await this.prisma.conversationPrompt.delete({ where: { name: newPrompt.name } }).catch(delErr => console.error(`Failed to rollback prompt ${newPrompt.name}`, delErr));
        ...
        const finalPrompt = await this.prisma.conversationPrompt.update({
    ...
    findAll(): Promise<ConversationPrompt[]> { // Devolvemos tipo base, el controlador puede mapear si necesita más
        return this.prisma.conversationPrompt.findMany({
            include: {
    ...
    async findOne(name: string): Promise<PromptWithRelations> {
        const prompt = await this.prisma.conversationPrompt.findUnique({
            where: { name },
            include: {
                tactic: true,
        ...
        if (!prompt) {
            throw new NotFoundException(`ConversationPrompt with NAME "${name}" not found`);
        }
        return prompt as PromptWithRelations;
    }

    // Refactorizado: Actualiza metadatos del Prompt (desc, tactic, tags)
    async update(name: string, updateDto: UpdateConversationPromptDto): Promise<PromptWithRelations> {
        const { tacticId, tags, ...restData } = updateDto;

        let data: Prisma.ConversationPromptUpdateInput = { ...restData }; // description
        ...
        try {
            ...
            await this.prisma.conversationPrompt.update({
                where: { name },
                data,
            });
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                if (error.code === 'P2025') {
                    ...
                    throw new NotFoundException(`ConversationPrompt with NAME "${name}" not found, or related Tactic/Tag could not be connected/found.`);
    ...
    async remove(name: string): Promise<ConversationPrompt> {
        ...
        try {
            return await this.prisma.$transaction(async (tx) => {
                ...
                await tx.conversationPrompt.update({
                    where: { name },
                    data: { tags: { set: [] } },
                ...
                const versions = await tx.conversationPromptVersion.findMany({ where: { promptId: name }, select: { id: true } });
                if (versions.length > 0) {
                    const versionIds = versions.map(v => v.id);
                    await tx.conversationPromptAssetLink.deleteMany({ where: { promptVersionId: { in: versionIds } } });
                    await tx.promptTranslation.deleteMany({ where: { versionId: { in: versionIds } } });
                    await tx.conversationPromptVersion.deleteMany({ where: { promptId: name } });
                }

                // 3. Finalmente, borrar el prompt
                const deletedPrompt = await tx.conversationPrompt.delete({
                    where: { name },
                });
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
                ...
                throw new NotFoundException(`ConversationPrompt with NAME "${name}" not found for deletion.`);
            }
    ...
    async createVersion(promptName: string, createVersionDto: CreatePromptVersionDto): Promise<ConversationPromptVersion> {
        const { promptText, versionTag, changeMessage, assetLinks, initialTranslations } = createVersionDto;

        const promptExists = await this.prisma.conversationPrompt.findUnique({ where: { name: promptName }, select: { name: true } });
        if (!promptExists) {
            throw new NotFoundException(`ConversationPrompt with NAME "${promptName}" not found.`);
        }

        try {
            const newVersion = await this.prisma.conversationPromptVersion.create({
                data: {
                    promptText,
                    versionTag,
                    changeMessage,
    ...
    async activateVersion(promptName: string, versionId: string): Promise<PromptWithRelations> {
        const versionExists = await this.prisma.conversationPromptVersion.findUnique({
    ...
    async addOrUpdateTranslation(versionId: string, translationDto: CreateOrUpdatePromptTranslationDto): Promise<PromptTranslation> {
        ...
        const versionExists = await this.prisma.conversationPromptVersion.findUnique({ where: { id: versionId }, select: { id: true } });
        if (!versionExists) {
        ...
        try {
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
                throw new NotFoundException(`ConversationPromptVersion with ID "${versionId}" not found during translation upsert.`);
            }
