...
export class RegionController {
  ...
  async update(
    ...
    const projectId = req.projectId;
    this.logger.debug(
      `[update] Received PATCH for projectId: ${projectId}, languageCode: ${languageCode}. Body: ${JSON.stringify(updateRegionDto, null, 2)}`,
    );
  ...
  async remove(
    ...
    const projectId = req.projectId;
    const removedRegion = await this.regionService.remove(
      languageCode,
      projectId,
    );
