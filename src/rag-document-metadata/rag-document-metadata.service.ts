...
export class RagDocumentMetadataService {
    ...
    private async validateRegionOwnership(regionId: string, projectId: string): Promise<void> {
        ...
        if (region.projectId !== projectId) {
            throw new BadRequestException(`Region with ID "${regionId}" does not belong to project "${projectId}".`);
        }
    }
    ...
    async create(createDto: CreateRagDocumentMetadataDto, projectId: string): Promise<RagDocumentMetadata> {
        ...
        if (existingDoc) {
            throw new ConflictException(`A RagDocumentMetadata with documentName "${documentName}" already exists in project "${projectId}".`);
        }
        ...
        try {
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                if (error.code === 'P2002') {
                    throw new ConflictException(`RagDocumentMetadata creation failed due to a unique constraint. Likely on documentName for this project.`);
                }
                if (error.code === 'P2025') {
                    throw new NotFoundException(`Failed to create RAG metadata: Referenced project or region not found.`);
                }
            }
            console.error(`Error creating RAG metadata for document "${documentName}" in project ${projectId}:`, error);
            throw error;
        }
    }
    ...
    async update(id: string, updateDto: UpdateRagDocumentMetadataDto, projectId: string): Promise<RagDocumentMetadata> {
        ...
        if (Object.keys(data).length === 0 && regionId === undefined && documentName === undefined) {
        ...
        try {
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
                const message = error.meta?.cause ?? `Record to update not found or related region not found. Ensure IDs are CUIDs.`;
                throw new NotFoundException(String(message));
            }
            if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
                throw new ConflictException(`Update failed due to a unique constraint violation. Likely on documentName for this project.`);
            }
            console.error(`Error updating RAG Metadata "${id}" in project ${projectId}:`, error);
            throw error;
        }
    }
