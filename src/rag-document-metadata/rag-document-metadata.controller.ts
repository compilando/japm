...
export class RagDocumentMetadataController {
    ...
    @ApiOperation({ summary: 'Crear metadatos para un documento RAG dentro de un proyecto' })
    ...
    @Get()
    @ApiOperation({ summary: 'Obtener todos los metadatos de documentos RAG de un proyecto' })
    @ApiParam({ name: 'projectId', description: 'ID del proyecto', type: String })
    ...
    @ApiParam({ name: 'projectId', description: 'ID del proyecto', type: String })
    @ApiParam({ name: 'metadataId', description: 'ID de los metadatos (CUID)', type: String })
    @ApiResponse({ status: 200, description: 'Metadatos encontrados.', type: RagDocumentMetadataResponse })
    @ApiResponse({ status: 404, description: 'Proyecto o Metadatos no encontrados.' })
    findOne(@Req() req: RequestWithProject, @Param('metadataId') id: string): Promise<RagDocumentMetadata> {
        const projectId = req.projectId;
    ...
    @ApiOperation({ summary: 'Actualizar metadatos por ID dentro de un proyecto' })
    @ApiParam({ name: 'projectId', description: 'ID del proyecto', type: String })
    @ApiParam({ name: 'metadataId', description: 'ID a actualizar', type: String })
    ...
    @ApiResponse({ status: 404, description: 'Proyecto, Metadatos o Región referenciada no encontrada.' })
    @ApiResponse({ status: 400, description: 'Datos inválidos.' })
    ...
    remove(@Req() req: RequestWithProject, @Param('metadataId') id: string): Promise<RagDocumentMetadata> {
        const projectId = req.projectId;
