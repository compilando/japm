...
export class MarketplaceService {
    ...
    async getPublishedPrompts(tenantId: string, params: MarketplaceQueryParams): Promise<PromptVersion[]> {
        ...
        this.logger.log(`Fetching published prompts for Tenant ID: ${tenantId} with params: ${JSON.stringify(params)}`);
        ...
        try {
            ...
            this.logger.log(`Found ${prompts.length} published prompts for Tenant ID: ${tenantId} with params: ${JSON.stringify(params)}`);
            return prompts;
        } catch (error) {
            this.logger.error(`Error fetching published prompts for Tenant ID: ${tenantId} - ${error.message}`, error.stack);
    ...
    async getPublishedAssets(tenantId: string, params: MarketplaceQueryParams): Promise<PromptAssetVersion[]> {
        ...
        this.logger.log(`Fetching published assets for Tenant ID: ${tenantId} with params: ${JSON.stringify(params)}`);
        ...
        try {
            ...
            this.logger.log(`Found ${assets.length} published assets for Tenant ID: ${tenantId} with params: ${JSON.stringify(params)}`);
            return assets;
        } catch (error) {
            this.logger.error(`Error fetching published assets for Tenant ID: ${tenantId} - ${error.message}`, error.stack);
            throw error;
        }
    }
} 